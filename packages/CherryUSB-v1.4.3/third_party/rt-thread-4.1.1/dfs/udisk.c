...
USB_NOCACHE_RAM_SECTION USB_MEM_ALIGNX uint8_t msc_sector_read[512 * 32];
USB_NOCACHE_RAM_SECTION USB_MEM_ALIGNX uint8_t msc_sector_write[512 * 32];
                               ...
                               rt_size_t size)
{
    ...
    buf_size = size * msc_class->blocksize;

    if (buf_size > sizeof(msc_sector_read))
    {
        rt_kprintf("sorry, it is over buffer size. (%d>%d)\n", buf_size, sizeof(msc_sector_read));
        rt_kprintf("Please enlarge msc_sector_read buffer size.\n");
        RT_ASSERT(0);
    }

    ret = usbh_msc_scsi_read10(msc_class, pos, (const uint8_t *)&msc_sector_read[0], size);
    if (ret != RT_EOK)
    {
    ...
    rt_memcpy(buffer, msc_sector_read, buf_size);
                                ...
                                rt_size_t size)
{
    ...
    if (buf_size > sizeof(msc_sector_write))
    {
        rt_kprintf("sorry, it is over buffer size. (%d>%d)\n", buf_size, sizeof(msc_sector_write));
        rt_kprintf("Please enlarge msc_sector_write buffer size.\n");
        RT_ASSERT(0);
    }

    rt_memcpy(msc_sector_write, buffer, buf_size);
    ret = usbh_msc_scsi_write10(msc_class, pos, (rt_uint8_t *)&msc_sector_write[0], size);
    if (ret != RT_EOK)
    {
...
rt_err_t rt_udisk_run(struct usbh_msc *msc_class)
{
    ...
    char dname[8];
    char sname[8];
    rt_uint8_t max_lun, sense[18], inquiry[36];
    ...
    memset(stor_r, 0, sizeof(struct ustor));

    ret = usbh_msc_scsi_read10(msc_class, 0, msc_sector_read, 1);
    if (ret != RT_EOK)
    {
    ...
    for (i = 0; i < MAX_PARTITION_COUNT; i++)
    {
        ...
        ret = dfs_filesystem_get_partition(&part[i], msc_sector_read, i);
        if (ret == RT_EOK)
        {
