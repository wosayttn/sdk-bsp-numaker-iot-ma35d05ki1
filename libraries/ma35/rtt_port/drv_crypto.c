...
#if ENABLE_DEBUG
    #define CRYPTO_DBGMSG   rt_kprintf
#else
...
static rt_sem_t s_psSemPRNG, s_psSemAES,  s_psSemECC, s_psSemHMAC;

static void CRYPTO_IRQHandler(void)
{
    if (PRNG_GET_INT_FLAG(CRYPTO))
    {
        PRNG_CLR_INT_FLAG(CRYPTO);

        rt_sem_release(s_psSemPRNG);
    }

    if (CRYPTO->INTSTS & CRYPTO_INTSTS_AESEIF_Msk)
    {
        CRYPTO_DBGMSG("AESERRIF is set!!  0x%x, 0x%x\n", CRYPTO->INTSTS, CRYPTO->AES_STS);
        if ((CRYPTO->AES_CNT & CRYPTO_AES_CNT_CNT_Msk) && (CRYPTO->AES_STS & CRYPTO_AES_STS_CNTERR_Msk))
        {
            CRYPTO_DBGMSG("    Not an error. AES_CNT is not multiple of 16.\n");
        }
        else
        {
            //AESERR
        }
        CRYPTO->INTSTS = CRYPTO_INTSTS_AESEIF_Msk;

        rt_sem_release(s_psSemAES);
    }

    if (CRYPTO->INTSTS & CRYPTO_INTSTS_AESIF_Msk)
    {
        CRYPTO_DBGMSG("AESIF!\n");
    ...
    {
        CRYPTO_DBGMSG("SHAERRIF is set!!\n");
        CRYPTO->INTSTS = CRYPTO_INTSTS_HMACEIF_Msk;

        rt_sem_release(s_psSemHMAC);
    }
    ...
    if (CRYPTO->INTSTS & CRYPTO_INTSTS_ECCIF_Msk)
    {
        ...
        CRYPTO_DBGMSG("ECC done IRQ.\n");

        rt_sem_release(s_psSemECC);
    }

    if (CRYPTO->INTSTS & CRYPTO_INTSTS_ECCEIF_Msk)
    {
        CRYPTO->INTSTS = CRYPTO_INTSTS_ECCEIF_Msk;
        CRYPTO_DBGMSG("ECCERRIF is set!!\n");
        CRYPTO_DBGMSG("ECC_STS = 0x%x\n", CRYPTO->ECC_STS);
...
int PRNG_ReSeed(CRYPTO_T *crypto, uint32_t u32Seed)
{
    ...
    crypto->PRNG_SEED = u32Seed;
    crypto->PRNG_CTL |= crypto->PRNG_CTL | CRYPTO_PRNG_CTL_SEEDRLD_Msk | CRYPTO_PRNG_CTL_START_Msk;

    result = rt_sem_control(s_psSemPRNG, RT_IPC_CMD_RESET, 0);
    RT_ASSERT(result == RT_EOK);

    result = rt_sem_take(s_psSemPRNG, RT_WAITING_FOREVER);
    RT_ASSERT(result == RT_EOK);
...
/**
  ...
  * @return None
  */
void AES_Open(CRYPTO_T *crypto, uint32_t u32EncDec, uint32_t u32OpMode,
...
int AES_Start(CRYPTO_T *crypto, int is_sm4, uint32_t u32DMAMode)
{
    ...
    if (is_sm4)
        crypto->AES_CTL |= CRYPTO_AES_CTL_SM4EN_Msk;

    crypto->AES_CTL |= (CRYPTO_AES_CTL_START_Msk | (u32DMAMode << CRYPTO_AES_CTL_DMALAST_Pos));

    result = rt_sem_control(s_psSemAES, RT_IPC_CMD_RESET, 0);
    RT_ASSERT(result == RT_EOK);

    result = rt_sem_take(s_psSemAES, RT_WAITING_FOREVER);
    RT_ASSERT(result == RT_EOK);
...
/**
  ...
  * @return None
  */
void AES_SetDMATransfer(CRYPTO_T *crypto, uint32_t u32FBIAddr, uint32_t u32FBOAddr,
                        uint32_t u32SrcAddr, uint32_t u32DstAddr, uint32_t u32TransCnt)
{
    crypto->AES_FBADDR = 0;
    if (u32FBIAddr != 0)
    {
        crypto->AES_CTL |= CRYPTO_AES_CTL_FBIN_Msk;
        crypto->AES_FBADDR = u32FBIAddr;
    }
    if (u32FBOAddr != 0)
    {
        crypto->AES_CTL |= CRYPTO_AES_CTL_FBOUT_Msk;
        crypto->AES_FBADDR = u32FBOAddr;
    }
...
int SHA_Start(CRYPTO_T *crypto, uint32_t u32DMAMode)
{
    ...
    result = rt_sem_control(s_psSemHMAC, RT_IPC_CMD_RESET, 0);
    RT_ASSERT(result == RT_EOK);

    result = rt_sem_take(s_psSemHMAC, RT_WAITING_FOREVER);
    RT_ASSERT(result == RT_EOK);
...
void SHA_Read(CRYPTO_T *crypto, uint32_t u32Digest[])
{
    ...
    for (i = 0U; i < wcnt; i++)
    {
        u32Digest[i] = crypto->HMAC_DGST[i];
    }
}
...
#define ECCOP_POINT_MUL     (0x0U << CRYPTO_ECC_CTL_ECCOP_Pos)
#define ECCOP_MODULE        (0x1U << CRYPTO_ECC_CTL_ECCOP_Pos)
#define ECCOP_POINT_ADD     (0x2U << CRYPTO_ECC_CTL_ECCOP_Pos)
#define ECCOP_POINT_DOUBLE  (0x0U << CRYPTO_ECC_CTL_ECCOP_Pos)

#define MODOP_DIV           (0x0U << CRYPTO_ECC_CTL_MODOP_Pos)
#define MODOP_MUL           (0x1U << CRYPTO_ECC_CTL_MODOP_Pos)
#define MODOP_ADD           (0x2U << CRYPTO_ECC_CTL_MODOP_Pos)
#define MODOP_SUB           (0x3U << CRYPTO_ECC_CTL_MODOP_Pos)
...
int ecc_init_curve(CRYPTO_T *crypto, E_ECC_CURVE ecc_curve);
...
void Hex2Reg(char input[], uint32_t volatile reg[])
{
    ...
    {
        val32 = 0U;
        for (i = 0U; (i < 8U) && (si >= 0); i++)
        {
            hex = ch2hex(input[si]);
            val32 |= (uint32_t)hex << (i * 4U);
            si--;
        }
        reg[ri++] = val32;
    }
...
void Hex2RegEx(char input[], uint32_t volatile reg[], int shift)
{
    ...
    {
        val32 = 0U;
        for (i = 0U; (i < 8U) && (si >= 0L); i++)
        {
            hex = (uint32_t)ch2hex(input[si]);
            hex <<= shift;

            val32 |= (uint32_t)((hex & 0xFU) | carry) << (i * 4U);
            carry = (hex >> 4U) & 0xFU;
            si--;
        }
        reg[ri++] = val32;
...
static ECC_CURVE *get_curve(E_ECC_CURVE ecc_curve)
{
    ...
    for (i = 0U; i < sizeof(_Curve) / sizeof(ECC_CURVE); i++)
    {
...
int ecc_init_curve(CRYPTO_T *crypto, E_ECC_CURVE ecc_curve)
{
    ...
    if (ret == 0)
    {
        ...
        {
            crypto->ECC_N[0] = 0x1U;
            crypto->ECC_N[(pCurve->key_len) / 32] |= (1U << ((pCurve->key_len) % 32));
            crypto->ECC_N[(pCurve->irreducible_k1) / 32] |= (1U << ((pCurve->irreducible_k1) % 32));
            crypto->ECC_N[(pCurve->irreducible_k2) / 32] |= (1U << ((pCurve->irreducible_k2) % 32));
            crypto->ECC_N[(pCurve->irreducible_k3) / 32] |= (1U << ((pCurve->irreducible_k3) % 32));
        }
        else
        {
...
int ECC_IsPrivateKeyValid(CRYPTO_T *crypto, E_ECC_CURVE ecc_curve, char private_k[])
{
    ...
    for (i = 0U; i < __strlen(private_k); i++)
    {
...
int  ECC_GeneratePublicKey(CRYPTO_T *crypto, E_ECC_CURVE ecc_curve, char *private_k, char public_k1[], char public_k2[])
{
    ...
    if (ret == 0)
    {
        ...
        if (ecc_curve == CURVE_25519)
        {
            ...
            crypto->ECC_CTL |= CRYPTO_ECC_CTL_CSEL_Msk;
        }
        ...
        result = rt_sem_control(s_psSemECC, RT_IPC_CMD_RESET, 0);
        RT_ASSERT(result == RT_EOK);

        result = rt_sem_take(s_psSemECC, RT_WAITING_FOREVER);
        RT_ASSERT(result == RT_EOK);
...
int  ECC_GeneratePublicKey_KS(CRYPTO_T *crypto, E_ECC_CURVE ecc_curve, int k_ksnum, int is_ecdh, char public_k1[], char public_k2[])
{
    ...
    if (ret == 0)
    {
        ...
        {
            if (is_ecdh)
                crypto->ECC_KSCTL = (0x2U << CRYPTO_ECC_KSCTL_RSSRCK_Pos) | CRYPTO_ECC_KSCTL_ECDH_Msk |
                                    CRYPTO_ECC_KSCTL_RSRCK_Msk | (k_ksnum - 0x80);
            else
                crypto->ECC_KSCTL = (0x2U << CRYPTO_ECC_KSCTL_RSSRCK_Pos) |
                                    CRYPTO_ECC_KSCTL_RSRCK_Msk | (k_ksnum - 0x80);
        }
        else
        {
...
/**
  ...
  * @return  -1   "ecc_curve" value is invalid.
  */
int  ECC_Mutiply_KS(CRYPTO_T *crypto, E_ECC_CURVE ecc_curve, int x1_ksnum, char x1[], int y1_ksnum, char y1[],
...
/**
  ...
  */
int  ECC_GenerateSecretZ_KS(CRYPTO_T *crypto, E_ECC_CURVE ecc_curve, int k_ksnum, char *private_k,
...
/**
  ...
  * @return  -1   "ecc_curve" value is invalid.
  */
int  ECC_GenerateSignature(CRYPTO_T *crypto, E_ECC_CURVE ecc_curve, char *message,
...
/**
  ...
  * @return  -1   "ecc_curve" value is invalid.
  */
int  ECC_GenerateSignature_KS(CRYPTO_T *crypto, E_ECC_CURVE ecc_curve, char *message,
...
/**
  ...
  */
int  ECC_VerifySignature(CRYPTO_T *crypto, E_ECC_CURVE ecc_curve, char *message,
...
/**
  ...
  * @return  -3   KS error
  */
int  ECC_VerifySignature_KS(CRYPTO_T *crypto, E_ECC_CURVE ecc_curve, char *message,
...
#define REG_SYS_CHIPCFG      (SYS_BASE + 0x1F4)
    ...
    uint32_t u32ChipCfg_CRYPTODIS = u32ChipCfg & (1 << 5);
    uint32_t u32ChipCfg_TSIDIS = u32ChipCfg & (1 << 8);

    if (u32ChipCfg_CRYPTODIS)
    {
        rt_kprintf("CRYPTO is not enabled in Chip Configuration. Please enable it before using CRYPTO driver.\n");

        return -1;
    }

    if (u32ChipCfg_TSIDIS)
    {
        /* Enable Crypto engine clock */
        outpw(TSI_CLK_BASE + 0x4, inpw(TSI_CLK_BASE + 0x4) | (1 << 12));

        s_psSemPRNG = rt_sem_create("prng", 0, RT_IPC_FLAG_FIFO);
        RT_ASSERT(s_psSemPRNG != RT_NULL);

        s_psSemAES = rt_sem_create("aes", 0, RT_IPC_FLAG_FIFO);
        RT_ASSERT(s_psSemAES != RT_NULL);

        s_psSemECC = rt_sem_create("ecc", 0, RT_IPC_FLAG_FIFO);
        RT_ASSERT(s_psSemECC != RT_NULL);

        /* Enable interrupt. */
        rt_hw_interrupt_install(CRYPTO_IRQn, nu_crypto_isr, NULL, "Crypto");
        rt_hw_interrupt_umask(CRYPTO_IRQn);
