...
PPResult PPInit(PPInst *pPostPInst)
{
    ...
    *pPostPInst = NULL; /* return NULL instance for any error */
    ...
    if (!hwConfig.addr64Support && sizeof(void *) == 8)
    {
        ...
        // ychuang - ignore this // return (PP_PARAM_ERROR);
    }
...
static void PPDecStartPp(const void *postPInst, const DecPpInterface *decpp)
{
    ...
    if (decpp->croppedW != ppInImg->width)
    {
        ...
        sysprintf("PPDecStartPp # exit runtime error croppedW:%d != ppW:%d\n", decpp->croppedW, ppInImg->width);
        ppC->PPCombinedRet = PP_DEC_RUNTIME_ERROR;
        return;
    }
    ...
    {
        // PP_API_TRC("PPDecStartPp # exit runtime error\n");
        sysprintf("PPDecStartPp # exit runtime error croppedH %d != ppH:%d\n", decpp->croppedH, ppInImg->height);
        ppC->PPCombinedRet = PP_DEC_RUNTIME_ERROR;
        return;
    }
                                ...
                                const PPOutputBuffers *pBuffers)
{
    ...
    sysprintf("#PPDecSetMultipleOutput, pBuffers->nbrOfBuffers = %d\n", pBuffers->nbrOfBuffers);
    ...
    {
        sysprintf("pBuffers->ppOutputBuffers[i].bufferBusAddr = 0x%lx\n", pBuffers->ppOutputBuffers[i].bufferBusAddr);
