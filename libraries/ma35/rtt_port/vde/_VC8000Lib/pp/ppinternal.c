...
/*------------------------------------------------------------------------------
    ...
    Argument        : PPContainer * ppC
------------------------------------------------------------------------------*/
static void PPFlushRegs(PPContainer *ppC)
{
    i32 i;
    u32 offset = PP_X170_REG_START;
    u32 *ppRegs = ppC->ppRegs;
...
#error "Bad value specified for PP_X170_BUS_BURST_LENGTH"
    ...
    // ychuang ; force PP support 16X x 16K input
    ppC->jpeg16kSupport = 1; // hwCfg.webpSupport; /* flag reused for JPEG as well */
                  ...
                  u32 decLinked, u32 decType)
{
    ...
    {
        sysprintf("!!%s - ppOutImg->bufferBusAddr = 0x%lx\n", __func__, ppOutImg->bufferBusAddr);
