                             ...
                             u32 mvcFlag)
{
    ...
    if (pSeqParamSet->vuiParametersPresentFlag)
    {
        ...
        if (tmp == HANTRO_OK)
        {
            pSeqParamSet->vuiParameters->updateHRDParameterFlag = 1;
            pSeqParamSet->vuiParameters->errorHRDParameterFlag = 0;
        }
        else
        {
            pSeqParamSet->vuiParameters->updateHRDParameterFlag = 0;
            pSeqParamSet->vuiParameters->errorHRDParameterFlag = 1;
        }
        if (tmp == END_OF_STREAM)
        {
        ...
        if (pSeqParamSet->vuiParameters->bitstreamRestrictionFlag)
        {
                    ...
                    pSeqParamSet->maxDpbSize)
            {
                ...
                if (pSeqParamSet->vuiParameters->maxDecFrameBuffering >
                        pSeqParamSet->maxDpbSize && invalidDpbSize)
                    pSeqParamSet->maxDpbSize =
                        pSeqParamSet->vuiParameters->maxDecFrameBuffering;
                else
                {
...
u32 h264bsdCompareSeqParamSets(seqParamSet_t *pSps1, seqParamSet_t *pSps2)
{
    ...
    u32 i;
    vuiParameters_t *pVui1, *pVui2;
    hrdParameters_t *pHrd1, *pHrd2;
            ...
            pSps1->scalingMatrixPresentFlag == pSps2->scalingMatrixPresentFlag)
    {
        ...
        if (pSps1->vuiParametersPresentFlag)
        {
            pVui1 = pSps1->vuiParameters;
            pVui2 = pSps2->vuiParameters;

            if (pVui1->aspectRatioPresentFlag      != pVui2->aspectRatioPresentFlag ||
                    pVui1->overscanInfoPresentFlag     != pVui2->overscanInfoPresentFlag ||
                    pVui1->videoSignalTypePresentFlag  != pVui2->videoSignalTypePresentFlag ||
                    pVui1->chromaLocInfoPresentFlag    != pVui2->chromaLocInfoPresentFlag ||
                    pVui1->timingInfoPresentFlag       != pVui2->timingInfoPresentFlag ||
                    pVui1->nalHrdParametersPresentFlag != pVui2->nalHrdParametersPresentFlag ||
                    pVui1->vclHrdParametersPresentFlag != pVui2->vclHrdParametersPresentFlag ||
                    pVui1->picStructPresentFlag        != pVui2->picStructPresentFlag ||
                    pVui1->bitstreamRestrictionFlag    != pVui2->bitstreamRestrictionFlag)
                return 1;

            else if (pVui1->aspectRatioPresentFlag)
            {
                if (pVui1->aspectRatioIdc != pVui2->aspectRatioIdc)
                    return 1;
                else if (pVui1->aspectRatioIdc == ASPECT_RATIO_EXTENDED_SAR)
                {
                    if (pVui1->sarWidth != pVui2->sarWidth ||
                            pVui1->sarHeight != pVui2->sarHeight)
                    {
                        return 1;
                    }
                }
            ...
            else if (pVui1->overscanInfoPresentFlag)
            {
                if (pVui1->overscanAppropriateFlag != pVui2->overscanAppropriateFlag)
                    return 1;
            }

            else if (pVui1->videoSignalTypePresentFlag)
            {
                if (pVui1->videoFormat                  != pVui2->videoFormat ||
                        pVui1->videoFullRangeFlag           != pVui2->videoFullRangeFlag ||
                        pVui1->colourDescriptionPresentFlag != pVui2->colourDescriptionPresentFlag)
                    return 1;
                else if (pVui1->colourDescriptionPresentFlag)
                {
                    if (pVui1->colourPrimaries         != pVui2->colourPrimaries ||
                            pVui1->transferCharacteristics != pVui2->transferCharacteristics ||
                            pVui1->matrixCoefficients      != pVui2->matrixCoefficients)
                    {
                        return 1;
                    }
                }
            ...
            else if (pVui1->chromaLocInfoPresentFlag)
            {
                if (pVui1->chromaSampleLocTypeTopField    != pVui2->chromaSampleLocTypeTopField ||
                        pVui1->chromaSampleLocTypeBottomField != pVui2->chromaSampleLocTypeBottomField)
                    return 1;
            }

            else if (pVui1->timingInfoPresentFlag)
            {
                if (pVui1->numUnitsInTick     != pVui2->numUnitsInTick ||
                        pVui1->timeScale          != pVui2->timeScale ||
                        pVui1->fixedFrameRateFlag != pVui2->fixedFrameRateFlag)
                    return 1;
            }


            else if (pVui1->nalHrdParametersPresentFlag || pVui1->vclHrdParametersPresentFlag)
            {
                if (pVui1->nalHrdParametersPresentFlag)
                {
                    pHrd1 = &(pVui1->nalHrdParameters);
                    pHrd2 = &(pVui2->nalHrdParameters);
                }
                else
                {
                    pHrd1 = &(pVui1->vclHrdParameters);
                    pHrd2 = &(pVui2->vclHrdParameters);
                }

                if (pHrd1->cpbCnt                       != pHrd2->cpbCnt ||
                        pHrd1->bitRateScale                 != pHrd2->bitRateScale ||
                        pHrd1->cpbSizeScale                 != pHrd2->cpbSizeScale ||
                        pHrd1->initialCpbRemovalDelayLength != pHrd2->initialCpbRemovalDelayLength ||
                        pHrd1->cpbRemovalDelayLength        != pHrd2->cpbRemovalDelayLength ||
                        pHrd1->dpbOutputDelayLength         != pHrd2->dpbOutputDelayLength ||
                        pHrd1->timeOffsetLength             != pHrd2->timeOffsetLength)
                    return 1;
                else if (pHrd1->cpbCnt)
                {
                    for (u32 i = 0; i < pHrd1->cpbCnt; i++)
                    {
                        if (pHrd1->bitRateValue[i] != pHrd2->bitRateValue[i] ||
                                pHrd1->cpbSizeValue[i] != pHrd2->cpbSizeValue[i] ||
                                pHrd1->cbrFlag[i]      != pHrd2->cbrFlag[i])
                            return 1;
                    }
                }

                if (pVui1->lowDelayHrdFlag != pVui2->lowDelayHrdFlag)
                    return 1;
            }

            else if (pVui1->bitstreamRestrictionFlag)
            {
                if (pVui1->motionVectorsOverPicBoundariesFlag != pVui2->motionVectorsOverPicBoundariesFlag ||
                        pVui1->maxBytesPerPicDenom                != pVui2->maxBytesPerPicDenom ||
                        pVui1->maxBitsPerMbDenom                  != pVui2->maxBitsPerMbDenom ||
                        pVui1->log2MaxMvLengthHorizontal          != pVui2->log2MaxMvLengthHorizontal ||
                        pVui1->log2MaxMvLengthVertical            != pVui2->log2MaxMvLengthVertical ||
                        pVui1->numReorderFrames                   != pVui2->numReorderFrames ||
                        pVui1->maxDecFrameBuffering               != pVui2->maxDecFrameBuffering)
                    return 1;
            }
        }
