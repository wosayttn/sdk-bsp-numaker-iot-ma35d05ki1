...
const u8 *h264bsdFindNextStartCode(const u8 *pByteStream, u32 len)
{
    ...
    const u8 *pStrm = pByteStream;
    const u8 *start;
    u32 found = 0;
    u8 data = 0xff;
    u32 byteCount = 0;
    u32 zeroCount = 0; // run zero
    uword dataInWord = 0;
    i32 bitPos = 0;
    u32 align = (WORDWIDTH / 8) - ((g1_addr_t)pByteStream & WORDADDR_REMAINDER_MASK);
    u32 isLittleE = IsLittleEndian();
    ...
    while (align > byteCount)
    {
        ...
        if (data == 0)
            zeroCount++;
        else if ((data == 0x01) && (zeroCount >= 2))
        {
            pStrm = pStrm - MIN(zeroCount, 3) - 1;
            found = 1;
            break;
        }
        else
    ...
    if (found == 0)
    {
        while (len >= (byteCount + WORDWIDTH / 8))
        {
            dataInWord = *((uword *)pStrm);

            if (dataInWord == 0x00)
            {
                /* boost zero stuffing skip performance */
                zeroCount += (WORDWIDTH / 8);
                byteCount += (WORDWIDTH / 8);
                pStrm += (WORDWIDTH / 8);
            }
            else
            {
                ...
                do
                {
                    ...
                    if (isLittleE)
                        data = (u8)(dataInWord >> bitPos);
                    else
                        data = (u8)(dataInWord >> ((WORDWIDTH - 8) - bitPos));
                    ...
                    {
                        found = 1;
                        pStrm = pStrm + bitPos / 8 - MIN(zeroCount, 3) - 1;
                        break;
                    }
                    else
                        zeroCount = 0;
                ...
                while (bitPos < WORDWIDTH);

                if (found)
                    break;

                pStrm += (bitPos >> 3); // bit to byte
                byteCount += (bitPos >> 3);
            }
        }
...
void h264bsdExtractSecureInfo(strmData_t *pStrmData, g1_addr_t *secureAddress, u32 *secureLen)
{
    ...
    *secureAddress = h264bsdGetBits(pStrmData, 8);
    *secureAddress = (h264bsdGetBits(pStrmData, 8) << 8) + *secureAddress;
    *secureAddress = (h264bsdGetBits(pStrmData, 8) << 16) + *secureAddress;
    *secureAddress = (h264bsdGetBits(pStrmData, 8) << 24) + *secureAddress;
    *secureAddress = ((u64)h264bsdGetBits(pStrmData, 8) << 32) + *secureAddress;
    *secureAddress = ((u64)h264bsdGetBits(pStrmData, 8) << 40) + *secureAddress;
    *secureAddress = ((u64)h264bsdGetBits(pStrmData, 8) << 48) + *secureAddress;
    *secureAddress = ((u64)h264bsdGetBits(pStrmData, 8) << 56) + *secureAddress;

    *secureLen = h264bsdGetBits(pStrmData, 8);
    *secureLen = (h264bsdGetBits(pStrmData, 8) << 8) + *secureLen;
    *secureLen = (h264bsdGetBits(pStrmData, 8) << 16) + *secureLen;
    *secureLen = (h264bsdGetBits(pStrmData, 8) << 24) + *secureLen;
}
