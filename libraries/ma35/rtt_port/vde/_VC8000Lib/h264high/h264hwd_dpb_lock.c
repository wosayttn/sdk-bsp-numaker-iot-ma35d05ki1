...
#define DPB_TRACE(fmt, ...) sysprintf("%s(): " fmt,\
...
u32 GetFreePicBuffer(FrameBufferList *fbList, u32 *old_id, u32 *is_free)
{
          ...
          )
    {
        for (i = 0; i < MAX_FRAME_BUFFER_NUMBER; i++)
        {
            if (old_id[i] == 0xFF)
                continue;
            else if (fbList->fbStat[old_id[i]].nRefCount == 0)
                break;
        }
...
{
    if (fbList->fbStat[id].bUsed & FB_TEMP_OUTPUT)
    {
        ClearOutput(fbList, id);
    }
}
...
void RemoveOutputAll(FrameBufferList *fbList)
{
    ...
    for (i = 0; i < MAX_FRAME_BUFFER_NUMBER; i++)
    {
        if (fbList->fbStat[i].bUsed & FB_OUTPUT)
        {
            ClearOutput(fbList, i);
        }
    }
...
#ifdef USE_OUTPUT_RELEASE
        while (fbList->fbStat[i].nRefCount != 0 && !fbList->abort)
#else
...
void MarkIdFree(FrameBufferList *fbList, u32 id)
{
    ...
    if (fbList->fbStat[id].bUsed | FB_ALLOCATED)
    {
        fbList->fbStat[id].bUsed &= ~FB_ALLOCATED;
        if (fbList->fbStat[id].nRefCount == 0)
            fbList->freeBuffers++;
...
{
    if (fbList == NULL || !fbList->bInitialized)
        return;
...
{
    (void)DWLmemset(fbList->outFifo, 0, MAX_FRAME_BUFFER_NUMBER * sizeof(struct OutElement_));
    fbList->wrId = 0;
    fbList->rdId = 0;
    fbList->numOut = 0;
}
