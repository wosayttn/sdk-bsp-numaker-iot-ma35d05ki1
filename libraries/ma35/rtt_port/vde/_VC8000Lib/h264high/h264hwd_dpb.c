                  ...
                  u32 longTermFrameIdx, u32 picStruct)
{
    ...
    for (i = 0; i <= dpb->dpbSize; i++)
        if (IS_LONG_TERM_F(dpb->buffer[i]) &&
                ...
                dpb->buffer[i].frameNum != (u32)picNum)
        {
                         ...
                         u32 tiledMode, u32 picCodeType)
{
    ...
    else if (isIdr)
    {
        ...
        SET_STATUS(*currentOut, EMPTY, FRAME);
        ...
        {
            RemoveTempOutputAll(dpb->fbList);
    ...
    else
    {
        ...
        if (!markedAsLongTerm)
        {
            ...
            else
            {
                ...
                DEBUG_PRINT(("NO ROOM IN DPB, probably corrupted stream, frameNum %d\n", frameNum));
                for (i = 0; i <= dpb->dpbSize; i++)
                {
    ...
    }
    {
        ...
        if (currentOut->isFieldPic && secondField)
            currentOut->numErrMbs = (currentOut->numErrMbs > 0 ?
                                     currentOut->numErrMbs : (i32)numErrMbs);
        else
            currentOut->numErrMbs = numErrMbs;

        currentOut->tiledMode = tiledMode;
        currentOut->isFieldPic = image->picStruct != FRAME;
        currentOut->isBottomField = image->picStruct == BOTFIELD;
        if (!currentOut->isFieldPic)
            currentOut->picStruct = image->picStruct;
        else if (!secondField)
            currentOut->picStruct = (image->picStruct == BOTFIELD) ? TOPFIELD : BOTFIELD;
        else
            currentOut->picStruct = image->picStruct;

        if (image->picStruct == TOPFIELD)
        {
            currentOut->isIdr[0] = isIdr;
            currentOut->picCodeType[0] = picCodeType;
            currentOut->decodeId[0] = currentPicId;
        }
        else if (image->picStruct == BOTFIELD)
        {
            currentOut->isIdr[1] = isIdr;
            currentOut->picCodeType[1] = picCodeType;
            currentOut->decodeId[1] = currentPicId;
        }
        else /* FRAME */
        {
            currentOut->isIdr[0] =  currentOut->isIdr[1] = isIdr;
            currentOut->picCodeType[0] =  currentOut->picCodeType[1] = picCodeType;
            currentOut->decodeId[0] = currentOut->decodeId[1] = currentPicId;
        }
    }
    ...
    if (dpb->numRefFrames == dpb->maxRefFrames)
    {
        ...
        {
            for (j = i + 1; j < dpb->maxRefFrames; j++)
            {
                if (((dpb->buffer[i].status[0] == SHORT_TERM && dpb->buffer[i].status[1] == EMPTY) ||
                        ...
                        (dpb->buffer[i].picNum == dpb->buffer[j].picNum))
                {
...
void h264DpbUpdateOutputList(dpbStorage_t *dpb)
{
    ...
    if (dpb->noReordering)
    {
        ...
        dpbOut->isIdr[0] = currentOut->isIdr[0];
        dpbOut->isIdr[1] = currentOut->isIdr[1];
        dpbOut->picId = currentOut->picId;
        dpbOut->picCodeType[0] = currentOut->picCodeType[0];
        dpbOut->picCodeType[1] = currentOut->picCodeType[1];
        dpbOut->decodeId[0] = currentOut->decodeId[0];
        dpbOut->decodeId[1] = currentOut->decodeId[1];
        ...
        dpbOut->crop = currentOut->crop;
        dpbOut->picWidth = currentOut->picWidth;
        dpbOut->picHeight = currentOut->picHeight;
        dpbOut->topField = 0;
        dpbOut->picStruct = currentOut->picStruct;
        dpbOut->corruptedSecondField = currentOut->corruptedSecondField;

        if (currentOut->isFieldPic)
        {
            if (currentOut->status[0] == EMPTY || currentOut->status[1] == EMPTY || currentOut->corruptedSecondField)
            {
                dpbOut->fieldPicture = 1;
                dpbOut->topField = (currentOut->status[0] == EMPTY) ? 0 : 1;
                if (currentOut->corruptedSecondField)
                    dpbOut->topField = (dpbOut->picStruct == TOPFIELD) ? 1 : 0;
        ...
        MarkTempOutput(dpb->fbList, currentOut->memIdx);
    }
    else
    {
...
void *h264bsdAllocateDpbImage(dpbStorage_t *dpb)
{
    ...
    u32 i;
    u32 memIdx[MAX_FRAME_BUFFER_NUMBER];
    ...
    if (i > dpb->dpbSize)
    {
        ...
        DEBUG_PRINT(("DPB full! Corrupt stream. Flushing slot %d, picNum %d\n", i, picNum));
    }

    ASSERT(i <= dpb->dpbSize);
    dpb->currentOut = &dpb->buffer[i];

    if (IsBufferReferenced(dpb->fbList, dpb->currentOut->memIdx))
    {
        /* inidicate the buffer type: FB_ALLOCATED or FB_FREE? */
        u32 isBufferFree;
        u32 newID = GetFreePicBuffer(dpb->fbList, memIdx, &isBufferFree);
        if (newID == (u32)0xFFFFFFFFU)
        {
            //dpb->currentOut->memIdx = newID;
            dpb->currentOut->data = NULL;
            return dpb->currentOut->data;
        }
        if (newID != dpb->currentOut->memIdx)
        {
            if (isBufferFree)
            {
                SetFreePicBuffer(dpb->fbList, dpb->currentOut->memIdx);
                dpb->currentOut->memIdx = newID;
                dpb->currentOut->data = GetDataById(dpb->fbList, newID);
            }
            else
            {
                for (i = 0; i <= dpb->dpbSize; i++)
                {
                    if (dpb->buffer[i].memIdx == newID)
                        break;
                }
                dpb->currentOut = &dpb->buffer[i];
            }
    ...
    dpb->currentOut->decodeId[0] = dpb->currentOut->decodeId[1] = -1;
    dpb->currentOut->corruptedFirstFieldOrFrame = 0;
    dpb->currentOut->corruptedSecondField = 0;
    dpb->currentOut->picWidth = dpb->picWidth;
    dpb->currentOut->picHeight = dpb->picHeight;
...
{
    FrameBufferList *fbList = dpb->fbList;
    u32 i, oldDpbSize;

    ASSERT(pDpbParams->maxFrameNum);
    ASSERT(pDpbParams->dpbSize);
    ASSERT(pDpbParams->picSizeInMbs);
    ASSERT(pDpbParams->maxRefFrames <= MAX_NUM_REF_PICS);
    ASSERT(pDpbParams->maxRefFrames <= pDpbParams->dpbSize);

    dpb->picSizeInMbs = pDpbParams->picSizeInMbs;
    dpb->maxLongTermFrameIdx = NO_LONG_TERM_FRAME_INDICES;
    dpb->maxRefFrames = MAX(pDpbParams->maxRefFrames, 1);

    oldDpbSize = dpb->dpbSize;
    if (pDpbParams->noReordering)
        dpb->dpbSize = dpb->maxRefFrames;
    else
        dpb->dpbSize = pDpbParams->dpbSize;

    dpb->maxFrameNum = pDpbParams->maxFrameNum;
    dpb->noReordering = pDpbParams->noReordering;
    dpb->fullness = 0;
    dpb->numRefFrames = 0;
    dpb->prevRefFrameNum = 0;
    dpb->numOut = 0;
    ...
    /* DO NOT update: dpb->totBuffers */

    for (i = 0; i < dpb->totBuffers; i++)
    {
        u32 picBuffSize;

        if (pDpbParams->isHighSupported)
        {
            /* yuv picture + direct mode motion vectors */
            picBuffSize = pDpbParams->picSizeInMbs *
                          ((pDpbParams->monoChrome ? 256 : 384) + 64);
            dpb->dirMvOffset = pDpbParams->picSizeInMbs *
                               (pDpbParams->monoChrome ? 256 : 384);

            /* allocate 32 bytes for multicore status fields */
            /* locate it after picture and direct MV */
            dpb->syncMcOffset = picBuffSize;
            picBuffSize += 32;
        }
        else
        {
            picBuffSize = pDpbParams->picSizeInMbs * 384;
        }

        if (pDpbParams->enable2ndChroma && !pDpbParams->monoChrome)
        {
            dpb->ch2Offset = picBuffSize;
            picBuffSize += pDpbParams->picSizeInMbs * 128;
        }
    }

    if (oldDpbSize < dpb->dpbSize)
    {
        for (i = oldDpbSize + 1; i < dpb->dpbSize + 1; i++)
        {
            ...
            for (j = 0; j < MAX_FRAME_BUFFER_NUMBER; j++)
            {
                ...
                for (u32 k = 0; k < i; k++)
                {
                    if (dpb->picBuffers[j].busAddress == dpb->buffer[k].data->busAddress)
                    {
                        found = 1;
                        break;
                    }
            ...
            dpb->buffer[i].data = dpb->picBuffers + j;
            id = GetIdByData(fbList, (void *)dpb->buffer[i].data);
            MarkIdAllocated(fbList, id);
    ...
    {
        for (i = dpb->dpbSize + 1; i < oldDpbSize + 1; i++)
        {
            MarkIdFree(fbList, dpb->buffer[i].memIdx);
        }
    }
    return (HANTRO_OK);
}
    ...
    struct dpbInitParams *pDpbParams)
{
    ...
    (void) DWLmemset(dpb->picBuffID, FB_NOT_VALID_ID, sizeof(dpb->picBuffID));
    ...
    if (pDpbParams->nCores == 1)
    {
        /* single core configuration */
        if (pDpbParams->displaySmoothing)
            dpb->totBuffers += pDpbParams->noReordering ? 1 : dpb->dpbSize + 1;
        else if (pDpbParams->multiBuffPp)
            dpb->totBuffers++;
    }
    else
    {
        ...
        if (pDpbParams->displaySmoothing && !pDpbParams->noReordering)
        {
            /* at least double buffers for smooth output */
            if (dpb->totBuffers > pDpbParams->nCores)
            {
                dpb->totBuffers *= 2;
            }
            else
            {
                dpb->totBuffers += pDpbParams->nCores;
            }
        }
        else
        {
            ...
            if (!pDpbParams->mvcView)
                dpb->totBuffers += pDpbParams->nCores;
        }
    }
#ifdef USE_OUTPUT_RELEASE
    dpb->totBuffersReserved = dpb->totBuffers;
#endif
    dpb->outBuf = DWLmalloc((MAX_NUM_REF_PICS + 1) * sizeof(dpbOutPicture_t));
    ...
    for (i = 0; i < dpb->totBuffers; i++)
    {
...
#ifndef USE_EXTERNAL_BUFFER
        if (DWLMallocRefFrm(dwl, picBuffSize, dpb->picBuffers + i) != 0)
            return (MEMORY_ALLOCATION_ERROR);

        if (i < dpb->dpbSize + 1)
        {
            u32 id = AllocateIdUsed(dpb->fbList, dpb->picBuffers + i);
        ...
        else
        {
            ...
            u32 id = AllocateIdFree(dpb->fbList, dpb->picBuffers + i);
            if (id == FB_NOT_VALID_ID)
                return MEMORY_ALLOCATION_ERROR;
        ...
        if (pDpbParams->isHighSupported)
        {
            ...
            void *base = (char *)(dpb->picBuffers[i].virtualAddress) +
                         dpb->dirMvOffset;
            (void)DWLmemset(base, 0, pDpbParams->picSizeInMbs * 64);

            /* set all pictures statuses to available */
            base = (char *)(dpb->picBuffers[i].virtualAddress) +
                   dpb->syncMcOffset;
    ...
    struct dpbInitParams *pDpbParams)
{
    ...
    if ((dpb->useAdaptiveBuffers && (dpb->nExtBufSizeAdded >= dpb->nNewPicSize)) ||
            (!dpb->useAdaptiveBuffers && (dpb->picSizeInMbs == pDpbParams->picSizeInMbs)))
#else
    if (dpb->picSizeInMbs >= pDpbParams->picSizeInMbs)
#endif
    {
        u32 new_dpbSize, new_totBuffers;

        dpb->maxLongTermFrameIdx = NO_LONG_TERM_FRAME_INDICES;
        dpb->maxRefFrames = (pDpbParams->maxRefFrames != 0) ?
                            pDpbParams->maxRefFrames : 1;

        if (pDpbParams->noReordering)
            new_dpbSize = dpb->maxRefFrames;
        else
            new_dpbSize = pDpbParams->dpbSize;

        /* max DPB size is (16 + 1) buffers */
        new_totBuffers = new_dpbSize + 1;
        ...
        if (pDpbParams->nCores == 1)
        {
            /* single core configuration */
            if (pDpbParams->displaySmoothing)
                new_totBuffers += pDpbParams->noReordering ? 1 : new_dpbSize + 1;
            else if (pDpbParams->multiBuffPp)
                new_totBuffers++;
        }
        else
        {
            ...
            {
                /* at least double buffers for smooth output */
                if (new_totBuffers > pDpbParams->nCores)
                {
                    new_totBuffers *= 2;
                }
                else
                {
                    new_totBuffers += pDpbParams->nCores;
                }
            }
            else
            {
...
#ifdef USE_EXTERNAL_BUFFER
        if ((dpb->useAdaptiveBuffers && (new_totBuffers + dpb->nGuardSize <= dpb->totBuffers)) ||
                (!dpb->useAdaptiveBuffers && (dpb->dpbSize == new_dpbSize)))
#else
...
#ifdef USE_EXTERNAL_BUFFER
            h264bsdUpdateDpb(dpb, pDpbParams);
            dpb->bUpdated = 1;
#endif
            return (HANTRO_OK);
        }
...
u32 OutputPicture(dpbStorage_t *dpb)
{
    ...
    if (dpb->numOut == dpb->dpbSize + 1)
    {
        ...
        ClearOutput(dpb->fbList, dpb->outBuf[dpb->outIndexR].memIdx);
...
#if 0
    if (tmp->corruptedFirstFieldOrFrame)
    {
        if (!IS_REFERENCE_F(*tmp))
        {
            dpb->fullness--;
        }
        tmp->corruptedFirstFieldOrFrame = 0;
        return (HANTRO_OK);
    }
#endif
    ...
    dpbOut->isIdr[0] = tmp->isIdr[0];
    dpbOut->isIdr[1] = tmp->isIdr[1];
    dpbOut->picId = tmp->picId;
    dpbOut->picCodeType[0] = tmp->picCodeType[0];
    dpbOut->picCodeType[1] = tmp->picCodeType[1];
    dpbOut->decodeId[0] = tmp->decodeId[0];
    dpbOut->decodeId[1] = tmp->decodeId[1];
    dpbOut->numErrMbs = tmp->numErrMbs;
    dpbOut->interlaced = dpb->interlaced;
    dpbOut->fieldPicture = 0;
    dpbOut->memIdx = tmp->memIdx;
    dpbOut->tiledMode = tmp->tiledMode;
    dpbOut->crop = tmp->crop;
    dpbOut->picWidth = tmp->picWidth;
    dpbOut->picHeight = tmp->picHeight;
    dpbOut->topField = 0;
    dpbOut->picStruct = tmp->picStruct;
    dpbOut->corruptedSecondField = tmp->corruptedSecondField;

    if (tmp->isFieldPic)
    {
        if (tmp->status[0] == EMPTY || tmp->status[1] == EMPTY || tmp->corruptedSecondField)
        {
            dpbOut->fieldPicture = 1;
            dpbOut->topField = (tmp->status[0] == EMPTY) ? 0 : 1;
            if (tmp->corruptedSecondField)
                dpbOut->topField = (dpbOut->picStruct == TOPFIELD) ? 1 : 0;
    ...
    MarkTempOutput(dpb->fbList, tmp->memIdx);
...
dpbOutPicture_t *h264bsdDpbOutputPicture(dpbStorage_t *dpb)
{
    ...
    DEBUG_PRINT(("h264bsdDpbOutputPicture: index %d outnum %d\n", (dpb->numOut -
                 ((dpb->outIndexW - dpb->outIndexR + dpb->dpbSize + 1) % (dpb->dpbSize + 1))),
                 dpb->numOut));
...
#endif
            if (dpb->picBuffID[i] != FB_NOT_VALID_ID)
                ReleaseId(dpb->fbList, dpb->picBuffID[i]);
        }
    }
...
void DpbBufFree(dpbStorage_t *dpb, u32 i)
{
    ...
    if (!dpb->buffer[i].toBeDisplayed && dpb->fullness > 0)
        dpb->fullness--;
...
static u32  GetDpbOutputTime(dpbPicture_t *pic)
{
    ...
    if (pic->isFieldPic)
    {
        ...
        {
            time0 = (u32)(pic->dpbOutputTime[0] * 1000);
            time1 = (u32)(pic->dpbOutputTime[1] * 1000);
            return MAX(time0, time1);
        }
        else
        {
            time0 = (pic->status[0] == EMPTY ? 0xFFFFFFFF : (u32)(pic->dpbOutputTime[0] * 1000));
            time1 = (pic->status[1] == EMPTY ? 0xFFFFFFFF : (u32)(pic->dpbOutputTime[1] * 1000));
            return MIN(time0, time1);
        }
    }
    else
        return ((u32)(pic->dpbOutputTime[0] * 1000));
}
...
dpbPicture_t *FindSmallestDpbTime(dpbStorage_t *dpb)
{
    ...
    cpbRemovalTime = (u32)(dpb->cpbRemovalTime * 1000);
    picOrderCnt = 0x7FFFFFFF;
    tmp = NULL;
    tmppoc = NULL;
    ...
    for (i = 0; i <= dpb->dpbSize; i++)
    {
        /* TODO: currently only outputting frames, asssumes that fields of a
         * frame are output consecutively */
        dpbOutputTime = GetDpbOutputTime(&dpb->buffer[i]);
        if (dpb->buffer[i].toBeDisplayed && (dpbOutputTime <= cpbRemovalTime))
        {
            tmp = dpb->buffer + i;
            cpbRemovalTime = dpbOutputTime;
        }
    }
    if (tmp != NULL)
    {
...
void h264DpbStateReset(dpbStorage_t *dpb)
{
    ...
    //dpb->fbList->freeBuffers = 0;
#endif
    ResetOutFifoInList(dpb->fbList);
    (void) DWLmemset(dpb->buffer, 0, 17 * sizeof(dpbPicture_t));
    for (i = 0; i <= dpb->dpbSize; i++)
        dpb->buffer[i].numErrMbs = -1;
    (void) DWLmemset(dpb->picBuffID, FB_NOT_VALID_ID, sizeof(dpb->picBuffID));
    dpb->currentOut = NULL;
    dpb->cpbRemovalTime = 0;
    dpb->bumpingFlag = 0;
    dpb->currentOutPos = 0;
    dpb->maxLongTermFrameIdx = NO_LONG_TERM_FRAME_INDICES;
    dpb->numRefFrames = 0;
...
void h264DpbRecover(dpbStorage_t *dpb, u32 currFrameNum, i32 currPoc)
{
    ...
    u32 minRefFrameNum, maxRefFrameNum;

    ASSERT(dpb->tryRecoverDpb == HANTRO_TRUE);

    if (dpb->maxRefFrames <= currFrameNum)
    {
        minRefFrameNum = currFrameNum - dpb->maxRefFrames;
    }
    else
    {
        minRefFrameNum = currFrameNum + dpb->maxFrameNum - dpb->maxRefFrames;
    }

    if (currFrameNum + dpb->maxRefFrames >= dpb->maxFrameNum)
    {
        maxRefFrameNum = currFrameNum + dpb->maxRefFrames - dpb->maxFrameNum;
    }
    else
    {
        maxRefFrameNum = currFrameNum + dpb->maxRefFrames;
    }

    for (i = 0; i <= dpb->dpbSize; i++)
    {
        if (buffer + i == dpb->currentOut)
            continue;
#if 0
        else if (IS_SHORT_TERM_F(buffer[i]) &&
                 (((minRefFrameNum <= maxRefFrameNum) &&
                   (buffer[i].frameNum < minRefFrameNum || buffer[i].frameNum > maxRefFrameNum)) ||
                  ((minRefFrameNum > maxRefFrameNum) &&
                   (buffer[i].frameNum < minRefFrameNum && buffer[i].frameNum > maxRefFrameNum))))
#endif
            /* Error recover till next I slice, clear all reference pictures
               in dpb, just like IDR. */
            else if (IS_REFERENCE_F(dpb->buffer[i]))
            {
                buffer[i].status[0] = UNUSED;
                buffer[i].status[1] = UNUSED;
                buffer[i].toBeDisplayed = 0;
                DpbBufFree(dpb, i);
            }

            else if (IS_UNUSED(buffer[i], FRAME) && buffer[i].toBeDisplayed)
            {
                i32 diffPoc = (GET_POC(buffer[i]) > currPoc) ?
                              GET_POC(buffer[i]) - currPoc :
                              currPoc - GET_POC(buffer[i]);

                if (buffer[i].toBeDisplayed && diffPoc >= 64)
                {
                    buffer[i].toBeDisplayed = 0;
                    DpbBufFree(dpb, i);
                }
    ...
    dpb->tryRecoverDpb = HANTRO_FALSE;
}
...
void h264RemoveNoBumpOutput(dpbStorage_t *dpb, u32 size)
{
    ...
    for (i = 0; i < size; i++)
    {
        index = dpb->outIndexW - i - 1;
        if (index < 0)
            index += (dpb->dpbSize + 1);
        id = (u32)index;
        ClearTempOut(dpb->fbList, dpb->outBuf[id].memIdx);
    }
}
