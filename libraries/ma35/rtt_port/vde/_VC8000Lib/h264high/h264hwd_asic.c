                        ...
                        u32 mbs)
{
    ...
    if (pDecCont->h264ProfileSupport != H264_BASELINE_PROFILE)
    {
        ...
        {
            ret |= DWLMallocLinear(pDecCont->dwl, tmp,
    ...
    if (!pDecCont->asicRunning)
    {
        ...
        for (i = 0; i < 16; i++)
        {
            ...
            if (pAsicBuff->refPicList[i] == 0x0)
                SET_ADDR_REG2(pDecCont->h264Regs, refBase[i], refBase_msb[i],
        ...
        if (pDecCont->storage.view && !pDecCont->storage.nonInterViewRef)
        {
            ...
            if (pDecCont->storage.dpbs[0]->currentOut->isFieldPic)
                tmp |= 0x2;
            SET_ADDR_REG(pDecCont->h264Regs, HWIF_INTER_VIEW_BASE, tmp);
        ...
        if (pSliceHeader->fieldPicFlag && pSliceHeader->bottomFieldFlag)
        {
            ...
            {
                SET_ADDR_REG(pDecCont->h264Regs, HWIF_DEC_OUT_BASE,
        ...
        if (pDecCont->rlcMode)
        {
            ...
            SET_ADDR_REG(pDecCont->h264Regs, HWIF_MB_CTRL_BASE,
                         pAsicBuff->mbCtrl.busAddress);

            SET_ADDR_REG(pDecCont->h264Regs, HWIF_RLC_VLC_BASE,
        ...
        else
        {
            ...
            if (pAsicBuff->enableDmvAndPoc && pDecCont->h264ProfileSupport != 2)
            {
                ...
                SET_ADDR_REG(pDecCont->h264Regs, HWIF_DIR_MV_BASE,
...
skipped_high_profile:
                ...
                !pDecCont->storage.activeSps->monoChrome)
        {
            ...
            if (pSliceHeader->fieldPicFlag && pSliceHeader->bottomFieldFlag)
            {
                SET_ADDR_REG(pDecCont->h264Regs, HWIF_DEC_CH8PIX_BASE,
        ...
        if (pDecCont->h264ProfileSupport != H264_BASELINE_PROFILE)
        {
            ...
            SET_ADDR_REG(pDecCont->h264Regs, HWIF_QTABLE_BASE,
...
#ifdef USE_64BIT_ENV
        DWLWriteReg(pDecCont->dwl, pDecCont->coreID, 4 * 122, pDecCont->h264Regs[122]);
#endif
    ...
    if (!pDecCont->rlcMode)
    {
        ...
        {
            g1_addr_t last_read_address;
            u32 bytes_processed;
            const g1_addr_t start_address =
                pDecCont->hwStreamStartBus & (~DEC_X170_ALIGN_MASK);
            const u32 offset_bytes =
                pDecCont->hwStreamStartBus & DEC_X170_ALIGN_MASK;

            last_read_address =
                GET_ADDR_REG(pDecCont->h264Regs, HWIF_RLC_VLC_BASE);
...
static void H264RefreshRegs(decContainer_t *pDecCont)
{
    ...
    decRegs =  pDecCont->h264Regs + TOTAL_X170_ORIGIN_REGS;
...
void H264SetupVlcRegs(decContainer_t *pDecCont)
{
    ...
    if (pSliceHeader->fieldPicFlag)
    {
        ...
        for (i = 0; i < 32; i++)
        {
            ...
            if (pSliceHeader->bottomFieldFlag &&
                    pSliceHeader->numRefIdxL0Active > 1 &&
                    pDpb->currentOut == &pDpb->buffer[i / 2] &&
                    (pDpb->currentOut->picCodeType[0] == DEC_PIC_TYPE_I
                     && pDpb->currentOut->isIdr[0] == 0) &&
                    IS_P_SLICE(pSliceHeader->sliceType) &&
                    (i & 1) && pDecCont->picNumber == 1)
                tmp = h264bsdGetRefPicDataVlcMode(pDpb, i - 1, 1) != NULL;
            else
    ...
    for (i = 0; i < 16; i++)
    {
        ...
        if (pDecCont->asicBuff->refPicList[i])
            pDecCont->asicBuff->refPicList[i] |=
                (g1_addr_t)((diffPoc < itmp ? 0x1 : 0) | (pDpb->buffer[i].isFieldPic ? 0x2 : 0));
    }
...
void h264StreamPosUpdate(decContainer_t *pDecCont)
{
    ...
    if (pDecCont->secureMode)
    {
        ...
        tmp = (pDecCont->secureDataBusAddress & DEC_X170_ALIGN_MASK) * 8;
        ...
        tmp = pDecCont->secureDataBusAddress;   /* unaligned base */
        tmp &= (g1_addr_t)(~DEC_X170_ALIGN_MASK);  /* align the base */

        DEBUG_PRINT(("\tStream base   %08x\n", tmp));
        SET_ADDR_REG(pDecCont->h264Regs, HWIF_RLC_VLC_BASE, tmp);

        tmp = pDecCont->secureDataLen;   /* unaligned stream */
...
void H264ErrorRecover(decContainer_t *pDecCont)
{
    ...
    if (!pDecCont->rlcMode)
    {
        ...
        while ((k--) > 0)
        {
            if (dpb->buffer[k].data == pStorage->currImage->data)
                break;
        }
        ...
        for (i = 0; i < dpb->dpbSize; i++)
        {
            ...
            if (IS_REF_MISSING_F(buffer[i]) && i != k)
            {
                /* missing field of a reference picture */
                pStorage->dpb->tryRecoverDpb = HANTRO_TRUE;
            }

#ifdef DISCARD_ERROR_PICTURE
            if (buffer[i].numErrMbs && IS_REFERENCE_F(buffer[i]))
                break;
#else
            if (buffer[i].numErrMbs && IS_REFERENCE_F(buffer[i]))
            {
                i32 picOrderCnt = GET_POC(dpb->buffer[i]);
                i32 tmp = idx = 0x7FFFFFFF;
                u32 j;

                for (j = 0; j <= dpb->dpbSize; j++)
                {
                    if (j == i || (buffer[j].numErrMbs > 0 ||
                                   buffer[j].numErrMbs == -1) ||
                            (j == k && !dpb->buffer[j].isFieldPic) ||
                            (j == k && (dpb->buffer[j].isFieldPic &&
                                        !IS_REFERENCE_F(buffer[j]))))
                        continue;
                    else if (ABS(picOrderCnt - GET_POC(dpb->buffer[j])) < tmp)
                    {
                        if (j != k)
                        {
                            tmp = ABS(picOrderCnt - GET_POC(dpb->buffer[j]));
                            idx = j;
                        }
                ...
                if (idx == 0x7FFFFFFF)
                    ...
                    pAsicBuff->refPicList[i] = buffer[idx].data->busAddress;
            }
            else
                idx = i;
#endif
                ...
                !IS_I_SLICE(pStorage->sliceHeader->sliceType))
        {
            ...
            while ((j--) > 0)
            {
                ...
                if (dpb->outBuf[tmp].data == pStorage->currImage->data)
                {
                    dpb->outBuf[tmp].numErrMbs = pStorage->picSizeInMbs;
                    break;
                }
                tmp++;
            }
        ...
        {
            for (i = dpb->dpbSize; i < 16; i++)
                pAsicBuff->refPicList[i] = pAsicBuff->refPicList[idx];
        }
    }
