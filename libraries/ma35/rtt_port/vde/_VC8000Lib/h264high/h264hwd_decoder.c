                  ...
                  u32 picId, u32 *readBytes)
{
    ...
    if (pStorage->sei.bumpingFlag)
    {
        if (nalUnit.nalUnitType != 12)
        {
            if (h264bsdStoreSEIInfoForCurrentPic(pStorage) == HANTRO_OK)
            {
                pStorage->sei.computeTimeInfo.accessUnitSize = 0;
                pStorage->sei.bumpingFlag = 0;
                pStorage->dpb->bumpingFlag = 1;
            }
        }
    }
    ...
    if (pDecCont->skipNonReference && nalUnit.nalRefIdc == 0)
    {
        ...
        if (pStorage->picStarted && pStorage->activeSps != NULL)
        {
            if (pDecCont->rlcMode)  /* Error conceal in RLC mode */
            {
                ...
                if (!pStorage->validSliceInAccessUnit)
                {
                    ...
                    if (pStorage->currImage->data == NULL)
    ...
    if (!picReady)
    {
        ...
        case NAL_PIC_PARAM_SET:
            ...
            else
            {
                ...
                /* we have allocated new memory inside h264bsdStorePicParamSet */
                FREE(picParamSet.runLength);
                FREE(picParamSet.topLeft);
                FREE(picParamSet.bottomRight);
                FREE(picParamSet.sliceGroupId);
        ...
        case NAL_CODED_SLICE_EXT:
            ...
            if (h264bsdIsStartOfPicture(pStorage))
            {
                ...
                tmp = h264bsdActivateParamSets(pStorage, ppsId, sliceType,
            ...
            if (!IS_I_SLICE(pStorage->sliceHeader[1].sliceType) &&
                    pDecCont->picNumber == 0)
            {
                DEBUG_PRINT(("SKIP P/B frames before the first I frame\n"));
                pStorage->picStarted = HANTRO_FALSE;
                ret = H264BSD_RDY;
                goto NEXT_NAL;
            }
            ...
            if (h264bsdIsStartOfPicture(pStorage))
            {
                                                       ...
                                                       pStorage->aub->newPicture)))
                {
                    ...
                    if (pStorage->dpb->delayedOut != 0)
                    {
                        ...
                        pStorage->dpb->outBuf[pStorage->dpb->delayedId].topField = 0;   /* mark both fields available */
                ...
                else
                {
                            ...
                            !pDecCont->modeChange && !pDecCont->gapsCheckedForThis)
                    {
                        ...
                        /* Avoid framp gap error after SEEK */
                        if (pDecCont->picNumber == 0)
                            pStorage->dpb->prevRefFrameNum = pStorage->sliceHeader[1].frameNum - 1;
        ...
        case NAL_SEI:
            ...
            tmp = h264bsdDecodeSeiParameters(pStorage->sps, &strm, &pStorage->sei);
            if (tmp != HANTRO_OK)
            {
...
NEXT_NAL:
    if (!pDecCont->rlcMode)
    {
        ...
        {
            if (nalUnit.nalUnitType == 12)
                pStorage->sei.computeTimeInfo.accessUnitSize += *readBytes;
        }
    }
    return ret;
...
void h264bsdCroppingParams(storage_t *pStorage, H264CropParams *pCrop)
{
    ...
    if (sps && sps->frameCroppingFlag)
    {
        ...
        pCrop->cropLeftOffset = tmp1 * sps->frameCropLeftOffset;
        pCrop->cropOutWidth = 16 * sps->picWidthInMbs -
                              tmp1 * (sps->frameCropLeftOffset + sps->frameCropRightOffset);

        pCrop->cropTopOffset = tmp1 * tmp2 * sps->frameCropTopOffset;
        pCrop->cropOutHeight = 16 * sps->picHeightInMbs -
    ...
    else
    {
        ...
        pCrop->cropOutWidth = sps ? sps->picWidthInMbs * 16 : 0;
        pCrop->cropTopOffset = 0;
        pCrop->cropOutHeight = sps ? sps->picHeightInMbs * 16 : 0;
    }
    ...
    *skippedBytes = 0;
    ...
    while (1)
    {
                ...
                tmp == NAL_CODED_SLICE_EXT)
        {
            ...
            bitPos = strmData.bitPosInWord + frameNumLen - 1 - BIT_N;
