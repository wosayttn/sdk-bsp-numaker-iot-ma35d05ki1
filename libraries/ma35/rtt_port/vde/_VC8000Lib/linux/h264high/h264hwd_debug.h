...
{ \
    if ((value) < (minBound) || (value) > (maxBound)) \
        {sysprintf("Warning: Value exceeds given limit(s)!\n");} \
}
...
{ \
    ...
    for (i = 0; i < (length); i++) \
        if ((array)[i] < (minBound) || (array)[i] > (maxBound)) \
            {sysprintf("Warning: Value [%d] exceeds given limit(s)!\n",i);} \
}
...
#ifdef _DEBUG_PRINT
    #include <stdio.h>
    #define DEBUG_PRINT(args) sysprintf args
#else
...
#ifdef _ERROR_PRINT
    #include <stdio.h>
    #define ERROR_PRINT(msg) sysprintf("ERROR: %s\n",msg)
#else
