...
static void PrintIrqType(u32 isPP, u32 coreID, u32 status)
{
    ...
    else
    {
        ...
        {
            sysprintf("DEC[%d] IRQ ABORT\n", coreID);
        }
        else if (status & DEC_IRQ_RDY)
        {
            sysprintf("DEC[%d] IRQ READY\n", coreID);
        }
        else if (status & DEC_IRQ_BUS)
        {
            sysprintf("DEC[%d] IRQ BUS ERROR\n", coreID);
        }
        else if (status & DEC_IRQ_BUFFER)
        {
            sysprintf("DEC[%d] IRQ BUFFER\n", coreID);
        }
        else if (status & DEC_IRQ_ASO)
        {
            sysprintf("DEC[%d] IRQ ASO\n", coreID);
        }
        else if (status & DEC_IRQ_ERROR)
        {
            sysprintf("DEC[%d] IRQ STREAM ERROR\n", coreID);
        }
        else if (status & DEC_IRQ_SLICE)
        {
            sysprintf("DEC[%d] IRQ SLICE\n", coreID);
        }
        else if (status & DEC_IRQ_TIMEOUT)
        {
            sysprintf("DEC[%d] IRQ TIMEOUT\n", coreID);
        }
        else
        {
            sysprintf("DEC[%d] IRQ UNKNOWN 0x%08x\n", coreID, status);
        }
    }
}
...
u32 DWLReadAsicID()
{
    ...
    //    goto end;
    //}
    hx170dec_ioctl(HX170DEC_IOCGHWOFFSET, &base);
    ...
    //    goto end;
    //}
    hx170dec_ioctl(HX170DEC_IOCGHWIOSIZE, &regSize);

    io = (u32 *)VDEC_BASE;  // DWLMapRegisters(fd, base, regSize, 0);
    //if(io == MAP_FAILED)
    //{
    ...
    /* to shadow regs */
    id = dwlShadowConfigRegs[0][0] = io[0];
...
void DWLReadAsicConfig(DWLHwConfig_t *pHwCfg)
{
    ...
    {
        sysprintf("HX170DEC_SYNTH_CFG (%d) = 0x%x\n", HX170DEC_SYNTH_CFG, dwlShadowConfigRegs[0][HX170DEC_SYNTH_CFG]);
        sysprintf("HX170DEC_SYNTH_CFG_2 (%d) = 0x%x\n", HX170DEC_SYNTH_CFG_2, dwlShadowConfigRegs[0][HX170DEC_SYNTH_CFG_2]);
        sysprintf("HX170PP_SYNTH_CFG (%d) = 0x%x\n", HX170PP_SYNTH_CFG, dwlShadowConfigRegs[0][HX170PP_SYNTH_CFG]);
        sysprintf("DWLReadAsicConfig config from shadow regs error!\n");
        ReadCoreConfig(dwlShadowConfigRegs[0], pHwCfg);
        return;
    }
#endif
    ...
    /* to shadow regs */
    dwlShadowConfigRegs[0][0] = io[0];
    dwlShadowConfigRegs[0][HX170DEC_SYNTH_CFG] = io[HX170DEC_SYNTH_CFG];
    dwlShadowConfigRegs[0][HX170DEC_SYNTH_CFG_2] = io[HX170DEC_SYNTH_CFG_2];
    dwlShadowConfigRegs[0][HX170PP_SYNTH_CFG] = io[HX170PP_SYNTH_CFG];
    dwlShadowConfigRegs[0][HX170DEC_FUSE_CFG] = io[HX170DEC_FUSE_CFG];
    dwlShadowConfigRegs[0][HX170PP_FUSE_CFG] = io[HX170PP_FUSE_CFG];
...
void DWLReadMCAsicConfig(DWLHwConfig_t pHwCfg[MAX_ASIC_CORES])
{
            ...
            dwlShadowConfigRegs[0][HX170PP_SYNTH_CFG] != 0x00000000)
    {
        for (i = 0; i < nCores; i++)
        {
            ReadCoreConfig(dwlShadowConfigRegs[i], pHwCfg + i);
            sysprintf("DWLReadMCAsicConfig config from shadow regs error!\n");
            return;
        }
    }
    ...
    //    goto end;
    //}
    hx170dec_ioctl(HX170DEC_IOC_MC_CORES, &nCores);
    ...
    //    goto end;
    //}
    hx170dec_ioctl(HX170DEC_IOC_MC_OFFSETS, mcRegBase);
    ...
    for (i = 0; i < nCores; i++)
    {
        io = (u32 *)VDEC_BASE;  // DWLMapRegisters(fd, mcRegBase[i], regSize, 0);
        //if(io == MAP_FAILED)
        // {
        //     DWL_DEBUG("failed to mmap registers\n");
        //    goto end;
        ...
        /* to shadow regs */
        dwlShadowConfigRegs[i][0] = io[0];
        dwlShadowConfigRegs[i][HX170DEC_SYNTH_CFG] = io[HX170DEC_SYNTH_CFG];
        dwlShadowConfigRegs[i][HX170DEC_SYNTH_CFG_2] = io[HX170DEC_SYNTH_CFG_2];
        dwlShadowConfigRegs[i][HX170PP_SYNTH_CFG] = io[HX170PP_SYNTH_CFG];
        dwlShadowConfigRegs[i][HX170DEC_FUSE_CFG] = io[HX170DEC_FUSE_CFG];
        dwlShadowConfigRegs[i][HX170PP_FUSE_CFG] = io[HX170PP_FUSE_CFG];
...
void DWLReadAsicFuseStatus(DWLHwFuseStatus_t *pHwFuseSts)
{
    ...
    {
        /* Decoder fuse configuration */
        ReadCoreFuse(dwlShadowConfigRegs[0], pHwFuseSts);
    }
...
#define PA_BLK_CNT          8192       /* VC8000 memory management block count */

struct vc8k_dma_blk
{
    void       *vaddr;      /* memory block virtual address */
    uint32_t   paddr;       /* memory block physical address */
    uint8_t    used;        /* 1: used; 0: free */
    int        contig_bcnt; /* number of contiguos blocks allocated */
};

static struct vc8k_dma_blk  _vc8k_mb_pool[PA_BLK_CNT];
static int  _pa_blk_size;  /* memory block size */
static int  _blk_alloc_cnt;
...
int vc8000_mem_init(uint32_t mem_base, uint32_t mem_size)
{
    ...
    _pa_blk_size = mem_size / PA_BLK_CNT;

    memset(_vc8k_mb_pool, 0, sizeof(_vc8k_mb_pool));

    _blk_alloc_cnt = 0;

    rmem_allocated = 0;
    for (i = 0; i < PA_BLK_CNT; i++)
    {
        _vc8k_mb_pool[i].used = 0;
        _vc8k_mb_pool[i].contig_bcnt = 0;
        _vc8k_mb_pool[i].paddr = mem_base + rmem_allocated;
        _vc8k_mb_pool[i].vaddr = nc_ptr(_vc8k_mb_pool[i].paddr);
        rmem_allocated += _pa_blk_size;
    }
    return 0;
}

void  vc8000_mem_deinit(void)
{
    memset(_vc8k_mb_pool, 0, sizeof(_vc8k_mb_pool));
    _blk_alloc_cnt = 0;
}

void *vc8000_mem_alloc(uint32_t *dma_addr, int size)
{
    int i, start;
    int found, wanted;
    ...
    start = -1;
    found = 0;
    wanted = (size + _pa_blk_size - 1) / _pa_blk_size;
    for (i = 0; i < PA_BLK_CNT - wanted + 1; i++)
    {
    ...
    if (found < wanted)
    {
        sysprintf("%s failed to allocate %d bytes!!! (%d / %d)\n", __func__, size, _blk_alloc_cnt, PA_BLK_CNT);
        return NULL;
    }
    ...
    _blk_alloc_cnt += wanted;

#ifdef MEMORY_USAGE_TRACE
    if (size < 1024)
    {
        sysprintf("%s - allocate %d bytes done. block %d, (%d / %d)\n", __func__, size, start, _blk_alloc_cnt, PA_BLK_CNT);
    }
    else
    {
        sysprintf("%s - allocate %d KB done. block %d, (%d / %d)\n", __func__, size / 1024, start, _blk_alloc_cnt, PA_BLK_CNT);
    }
#endif
...
int vc8000_mem_free(void *vaddr)
{
    int i, start, wanted;
    uint32_t  base, dma_addr;

    dma_addr = addr_s(vaddr);
    base = _vc8k_mb_pool[0].paddr;

    if ((dma_addr < base) || (dma_addr > base + PA_BLK_CNT * _pa_blk_size))
    {
        sysprintf("%s - invalid DMA address 0x%x!\n", __func__, (u32)dma_addr);
        return DWL_ERROR;
    }

    start = (dma_addr - base) / _pa_blk_size;
    if (_vc8k_mb_pool[start].paddr != dma_addr)
    {
        sysprintf("%s dma_addr not block aligned: 0x%x\n", __func__, (u32)dma_addr);
        return DWL_ERROR;
    }
    wanted = _vc8k_mb_pool[start].contig_bcnt;

    if ((dma_addr + wanted * _pa_blk_size) > (base + PA_BLK_CNT * _pa_blk_size))
    {
        sysprintf("%s - invalid DMA address 0x%x!\n", __func__, (u32)dma_addr);
        return DWL_ERROR;
    }
...
#ifdef MEMORY_USAGE_TRACE
    sysprintf("%s free %d KB done. block %d, (%d / %d)\n", __func__, wanted * (_pa_blk_size / 1024), start, _blk_alloc_cnt, PA_BLK_CNT);
#endif
    return DWL_OK;
}
...
i32 DWLMallocRefFrm(const void *instance, u32 size, DWLLinearMem_t *info)
{
    ...
    info->virtualAddress = vc8000_mem_alloc(&dma_addr, size);
...
#ifdef MEMORY_USAGE_TRACE
    sysprintf("DWLMallocLinear\t%8d bytes \n", size);
#endif
    info->virtualAddress = vc8000_mem_alloc(&dma_addr, size);
    if (info->virtualAddress == NULL)
        return  DWL_ERROR;
...
void DWLFreeLinear(const void *instance, DWLLinearMem_t *info)
{
    ...
    //assert(dec_dwl != NULL);
    //assert(info != NULL);
    vc8000_mem_free(info->virtualAddress);
}
...
//  core.regs += isPP ? 60 : 0;
...
#if 0  // dump registers
    {
        uint32_t   reg_base = 0x40290000;
        int        i;

        for (i = 0; i < 118; i++)
        {
            sysprintf("swreg%d - 0x%08x\n", i, inpw(reg_base + i * 4));
        }
    ...
    {
#if 1  // #ifdef DWL_SINGLE_CORE_SW_ONLY
    ...
    if (isPP)
    {
        sysprintf("POLLING PP\n");
    }
    else
    {
        sysprintf("POLLING DEC\n");
    }

    do
    {
        u32 irq_stats;
        const unsigned int usec = 100; /* 100 us polling interval */

        // sysprintf("POLLING HW STATUS...\n");
...
void *DWLmalloc(u32 n)
{
    ...
    return vc8000_mem_alloc(&dma_addr, n);
#else
