...
#define DIV_CEIL(a,b) (((a)+(b)-1)/(b))
    ...
    sysprintf("***** ref buffer mem model trace *****\n");
    sysprintf("latency             = %7d clk\n", pRefbu->currMemModel.latency);
    sysprintf("sequential          = %7d clk\n", pRefbu->currMemModel.nonseq);
    sysprintf("non-sequential      = %7d clk\n", pRefbu->currMemModel.seq);

    sysprintf("latency (mb)        = %7d\n", pRefbu->memAccessStats.latency);
    sysprintf("sequential (mb)     = %7d\n", pRefbu->memAccessStats.nonseq);
    sysprintf("non-sequential (mb) = %7d\n", pRefbu->memAccessStats.seq);

    sysprintf("bus-width           = %7d\n", busWidth);

    sysprintf("buffering cycles    = %7d\n", pRefbu->numCyclesForBuffering);
    sysprintf("buffer penalty      = %7d\n", pRefbu->bufferPenalty);
    sysprintf("avg cycles per mb   = %7d\n", pRefbu->avgCyclesPerMb);
...
i32 RefbuGetHitThreshold(refBuffer_t *pRefbu)
{
...
#ifdef REFBUFFER_TRACE
    i32 predMiss;
    sysprintf("***** ref buffer threshold trace *****\n");
#endif /* REFBUFFER_TRACE */
    ...
    if (divisor > 0)
    {
        ...
        predMiss = 4 * pRefbu->frmSizeInMbs - pRefbu->predIntraBlk -
                   pRefbu->predCoverage;
        sysprintf("predicted misses    = %7d\n", predMiss);
        sysprintf("data excess %%       = %7d\n", pRefbu->dataExcessMaxPct);
        sysprintf("divisor             = %7d\n", divisor);
#endif /* REFBUFFER_TRACE */
...
#ifdef REFBUFFER_TRACE
    sysprintf("target (clk)        = %7d\n", requiredHitsClk);
    sysprintf("target (data)       = %7d\n", requiredHitsData);
    sysprintf("***** ref buffer threshold trace *****\n");
               ...
               u32 picHeightInMbs, u32 supportFlags)
{
    ...
    if ((u32)pRefbu->picWidthInMbs == picWidthInMbs &&
            (u32)pRefbu->picHeightInMbs == picHeightInMbs)
    {
        return;
    }

#ifdef REFBUFFER_TRACE
    sysprintf("***** ref buffer initialized to %dx%d *****\n",
...
#ifdef REFBUFFER_TRACE
    sysprintf("***** ref buffer algorithm trace *****\n");
    sysprintf("predicted coverage  = %7d\n", cov);
    sysprintf("bus width in calc   = %7d\n", pRefbu->busWidthInBits);
    sysprintf("coverage history    = %7d%7d%7d\n",
              pRefbu->coverage[0],
              pRefbu->coverage[1],
              pRefbu->coverage[2]);
    sysprintf("enable              = %d (%8d<%8d)\n", enable, numCyclesForBuffering, tmp);
#endif /* REFBUFFER_TRACE */
...
#ifdef REFBUFFER_TRACE
    sysprintf("offset_x            = %7d\n", *pX);
    sysprintf("offset_y            = %7d (%d %d)\n", *pY, pRefbu->oy[0], pRefbu->oy[1]);
    sysprintf("***** ref buffer algorithm trace *****\n");
#endif /* REFBUFFER_TRACE */
...
void RefbuMvStatisticsB(refBuffer_t *pRefbu, u32 *regBase)
{
    ...
    if (pRefbu->fldCnt >= 2 &&
            ...
            (topFldCnt || botFldCnt))
    {
        pRefbu->fldHitsB[2][0] = pRefbu->fldHitsB[1][0];
        pRefbu->fldHitsB[2][1] = pRefbu->fldHitsB[1][1];
        pRefbu->fldHitsB[1][0] = pRefbu->fldHitsB[0][0];
        pRefbu->fldHitsB[1][1] = pRefbu->fldHitsB[0][1];
        if (GetDecRegister(regBase, HWIF_PIC_TOPFIELD_E))
        {
            pRefbu->fldHitsB[0][0] = topFldCnt;
            pRefbu->fldHitsB[0][1] = botFldCnt;
        }
        else
        {
            pRefbu->fldHitsB[0][0] = botFldCnt;
            pRefbu->fldHitsB[0][1] = topFldCnt;
        }
    }
                       ...
                       u32 *pMv, u32 directMvsAvailable, u32 isIntraPicture)
{
...
#ifdef REFBUFFER_TRACE
    sysprintf("***** ref buffer mv statistics trace *****\n");
    sysprintf("intra blocks        = %7d\n", numIntraBlk);
    sysprintf("top fields          = %7d\n", topFldCnt);
    sysprintf("bottom fields       = %7d\n", botFldCnt);
#endif /* REFBUFFER_TRACE */
    ...
    if (pRefbu->fldCnt > 0 &&
            ...
            (topFldCnt || botFldCnt))
    {
        pRefbu->fldHitsP[2][0] = pRefbu->fldHitsP[1][0];
        pRefbu->fldHitsP[2][1] = pRefbu->fldHitsP[1][1];
        pRefbu->fldHitsP[1][0] = pRefbu->fldHitsP[0][0];
        pRefbu->fldHitsP[1][1] = pRefbu->fldHitsP[0][1];
        if (GetDecRegister(regBase, HWIF_PIC_TOPFIELD_E))
        {
            pRefbu->fldHitsP[0][0] = topFldCnt;
            pRefbu->fldHitsP[0][1] = botFldCnt;
        }
        else
        {
            pRefbu->fldHitsP[0][0] = botFldCnt;
            pRefbu->fldHitsP[0][1] = topFldCnt;
        }
    }
    if (GetDecRegister(regBase, HWIF_FIELDPIC_FLAG_E))
        pRefbu->fldCnt++;
...
#ifdef REFBUFFER_TRACE
        sysprintf("calculated coverage = %7d\n", pRefbu->coverage[0]);
#endif /* REFBUFFER_TRACE */
...
#ifdef REFBUFFER_TRACE
        sysprintf("estimated coverage  = %7d\n", pRefbu->coverage[0]);
#endif /* REFBUFFER_TRACE */
...
#ifdef REFBUFFER_TRACE
    sysprintf("predicted intra blk = %7d\n", pRefbu->predIntraBlk);
    sysprintf("***** ref buffer mv statistics trace *****\n");
#endif /* REFBUFFER_TRACE */
                ...
                u32 refPicId0, u32 refPicId1, u32 flags)
{
    ...
    {
        if ((u32)pRefbu->prevLatency != pRefbu->currMemModel.latency)
        {
        ...
        if (flags & REFBU_DONT_USE_STATS)
            enable = pRefbu->picWidthInMbs > 16 ? 1 : 0;
        else
